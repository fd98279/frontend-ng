...
import { IInsiderTrade } from '../../../insider-trading/insider-trading';
...
export class InsiderTradesGridComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @Input() insiderTrades: IInsiderTrade[];
    @Output() insiderTradesEntrySelected: EventEmitter<IInsiderTrade[]> = new EventEmitter();
    public minDate: Date;
    public maxDate: Date;
    ...
    public handleRowSelectionChange(args) {
        this.insiderTradesEntrySelected.emit(args.newSelection);
    }
    ...
    private backgroundClasses = (rowData: any, columnKey: string): boolean => {
        return rowData.transactionAcquiredDisposed == 'A';
    }

    private backgroundClasssesPsPercentage = (rowData: any, columnKey: string): boolean => {
        return rowData.transactionAcquiredDisposed == 'D';
    }
