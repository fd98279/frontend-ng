...
import { NUMBERS_PATTERN, EMAIL_PATTERN } from '../../@auth/components';
...
export class UserComponent implements OnInit, OnDestroy {
  ...
  loadUserData() {
    ...
    if (isProfile) {
      ...
      this.loadUser();
      // Do not allow guest edit
      if ( this.userStore.getUser() && this.userStore.getUser().login !== 'guest123' ) {
        this.readonly = false;
        // this.setViewMode(UserFormMode.EDIT_SELF);
      } else {
        this.readonly = true;
        // this.setViewMode(UserFormMode.VIEW);
      }
    } else {
      if (id) {
  ...
  save() {
    if (!this.readonly) {
      ...
      this.toastrService.error('', `Updates not allowed!`, {
        positionClass: 'toast-bottom-center' });
    }
  }

  handleSuccessResponse() {
    this.toastrService.success('', `Item ${this.mode === UserFormMode.ADD ? 'created' : 'updated'}!`, {
      positionClass: 'toast-bottom-center' });
    this.back();
  }

  handleWrongResponse() {
    this.toastrService.error('', `This email has already taken!`, {
      positionClass: 'toast-bottom-center' });
  }
